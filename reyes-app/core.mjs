// Reyes Office — der Kern: das goldene Gehirn-Hologramm aus dem Jarvis-Dashboard (modules/home/BrainCore.tsx), 1:1 nach JavaScript
// übertragen (automatisch, nicht von Hand ändern: siehe reyes-app/build-core.md). Zustand von außen: core.set("idle"|"listening"|"thinking"|"speaking", Pegel 0..1).
// Unterschied zum Dashboard: Dort meldet die Live-Konsole Zustand und Pegel per Nachricht; hier setzt die App sie direkt.
export function createCore(cv) {
    const live = { state: "idle", level: 0 };
    ...
    const ring = (rx, ry, rot, dash, gap, col, w, ph) => {
        ...
        ctx.lineWidth = w;
        const step = dash + gap;
        for (let a = ph; a < ph + TAU; a += step) {
            ctx.beginPath();
            ctx.ellipse(0, 0, rx, ry, 0, a, a + dash);
            ctx.stroke();
        }
        ctx.restore();
    };
    function addImp(n) { for (let i = 0; i < n && links.length; i++)
        imps.push({ l: Math.floor(Math.random() * links.length), t: 0, d: Math.random() < 0.5 ? 1.3 : 1.9 }); }
    function frame(now) {
        const dt = Math.min(0.05, (now - last) / 1000);
        last = now;
        tt += dt;
        const m = live, fresh = m.state !== "idle";
        const speaking = fresh && m.state === "speaking", listening = fresh && m.state === "listening";
        const busy = (fresh && m.state === "thinking") || false;
        ...
        if (speaking && m.level > 0.28 && prevLv <= 0.28 && now - lastBeat > 140) {
            flash = 1;
            lastBeat = now;
            shock.push(0);
            addImp(18);
        ...
        flash *= Math.exp(-dt * 6);
        if (!speaking && Math.random() < dt * (busy ? 9 : 1.6))
            addImp(busy ? 3 : 1);
        ...
        g.addColorStop(0, `rgba(255,170,60,${0.34 + 0.3 * level + 0.2 * flash})`);
        g.addColorStop(0.4, `rgba(255,120,20,${0.1 + 0.08 * level})`);
        ...
        for (let i = 0; i < 120; i++) {
            ...
            ctx.strokeStyle = long ? "rgba(255,190,90,.75)" : "rgba(255,170,70,.32)";
            ctx.lineWidth = long ? 1.4 : 1;
            ctx.beginPath();
            ctx.moveTo(Math.cos(a) * r1, Math.sin(a) * r1 * 0.34);
            ctx.lineTo(Math.cos(a) * r2, Math.sin(a) * r2 * 0.34);
            ctx.stroke();
        }
        ctx.restore();
        ...
        for (let i = 0; i < 3; i++) {
            ...
            ctx.fillStyle = "rgba(255,225,160,.95)";
            ctx.beginPath();
            ctx.arc(cx + Math.cos(a) * rr, cy + Math.sin(a) * rr, 2.6, 0, TAU);
            ctx.fill();
        }
        for (let i = shock.length - 1; i >= 0; i--) {
            ...
            ctx.strokeStyle = `rgba(255,190,90,${0.55 * (1 - s)})`;
            ctx.lineWidth = 2 * (1 - s) + 0.5;
            ctx.beginPath();
            ctx.arc(cx, cy, R * (1.0 + s * 0.9), 0, TAU);
            ctx.stroke();
        }
        ctx.lineWidth = 0.8;
        for (let k = 0; k < 14; k++) {
            ...
            ctx.beginPath();
            for (let j = 0; j <= 28; j++) {
                const [x, y, z] = shape(u, 0.05 + (j / 28) * (Math.PI - 0.1));
                const q = proj(x, y, z, ca, sa, ct, st, sc);
                if (j)
                    ctx.lineTo(q[0], q[1]);
                else
                    ctx.moveTo(q[0], q[1]);
            }
            ctx.strokeStyle = `rgba(255,170,70,${0.07 + 0.06 * level})`;
            ctx.stroke();
        }
        for (let k = 1; k < 9; k++) {
            const v = (k / 9) * Math.PI;
            ...
            ctx.strokeStyle = `rgba(255,170,70,${0.08 + 0.06 * level})`;
            ctx.stroke();
        }
        const lp = (i) => proj(pts[i].x, pts[i].y, pts[i].z, ca, sa, ct, st, sc);
        ctx.strokeStyle = `rgba(255,190,100,${0.09 + 0.1 * level})`;
        ctx.beginPath();
        for (const l of links) {
            const a = lp(l.a), b = lp(l.b);
            if (a[2] < -0.1 && b[2] < -0.1)
                continue;
            ctx.moveTo(a[0], a[1]);
            ctx.lineTo(b[0], b[1]);
        ...
        cg.addColorStop(0, `rgba(255,236,190,${0.35 + 0.4 * level})`);
        ...
        for (let k = 0; k < 3; k++) {
            ...
            ctx.strokeStyle = `rgba(255,165,60,${0.6 - k * 0.15})`;
            ctx.lineWidth = 1.3 - k * 0.2;
            ctx.stroke();
        }
    ...
    if (reduce)
        ...
        raf = requestAnimationFrame(frame);
    return { set(state, level = 0) { live.state = state; live.level = Math.max(0, Math.min(1, level)); }, stop() { cancelAnimationFrame(raf); ro.disconnect(); } };
}
