...
here instead:
  ...
  * **google** — the user's real Google Calendar, and the **default**. Needs a
    one-time OAuth connect from ⚙ → PLUGIN SETTINGS; until that happens every
    call refuses with instructions instead of quietly booking somewhere the
    user never looks.
  * **local**  — a JSON store plus an .ics file per appointment, handed to
    ...
    exists for someone who explicitly wants an offline calendar — it is no
    longer a silent fallback, because an appointment nobody can find is worse
    than a clear refusal.

A third setting, `auto`, keeps the old behaviour for anyone who wants it: try
Google, and fall back to local (with the fallback said out loud) when it is
not connected.
...
def _pick_backend():
    ...
    choice = str(get_plugin_setting(NAMESPACE, "backend", "google") or "google").lower()
    local = LocalCalendar(
...
PLUGIN_SETTINGS = {
    ...
    "fields": [
        {"key": "backend", "label": "Calendar", "type": "choice",
         "options": ["google", "auto", "local"], "default": "google"},
...
PLUGIN = {
    ...
    "description": (
        ...
        "Writes to the user's real Google Calendar by default (needs a one-time connect from "
        "⚙ → PLUGIN SETTINGS); if it is not connected the tool says so rather than silently "
        "saving somewhere the user never looks. Use the 'reminder' tool instead when the user "
        "only wants a one-off notification at a time, with no appointment in their calendar."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
